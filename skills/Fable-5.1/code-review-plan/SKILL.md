...
2. **If the target is broad or unspecified** — Survey the entire project structure. Identify the most critical areas (core business logic, public API surface, security-sensitive code, complex modules) and prioritize them. Record the areas you did not review in Section 7 so the plan does not imply full coverage.
...
### Step 4b: Self-Verify Critical and Warning Findings

Before any Critical or Warning finding enters the plan, try to refute it yourself: re-open every cited `file:line`, confirm the code says what the finding claims, and trace the call path or run an existing test where that settles it. Drop findings that fail re-checking; mark findings you could not check as unverified in Section 7.
...
## 2. Review Findings
<Perspective is the Step 3 perspective that produced the finding: Correctness, Security, Performance, Architecture, Readability, or Testing.>

### Critical
- `file:line` — [Perspective] Description of the issue and why it matters.

### Warnings
- `file:line` — [Perspective] Description and recommended fix.

### Improvements
- `file:line` — [Perspective] Description and suggested improvement.

### Nits
- `file:line` — [Perspective] Minor suggestions.
