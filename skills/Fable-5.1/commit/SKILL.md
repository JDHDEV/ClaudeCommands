...
4. For each untracked file (enumerate them with `git ls-files --others --exclude-standard`, since `git status` collapses untracked directories and `-uall` is not allowed), read its contents (skip binaries and files over about 1 MB); `git diff` never shows untracked content, and Step 2 must scan it too.
5. Run `git log --oneline -10` to see recent commit message style and conventions.
...
Scan every changed line (staged + unstaged) and the contents of the untracked files read in Step 1 for the following problems:
...
Hits inside documentation that merely describes these patterns (for example a skill file or README that lists `TODO` or `.only` as things to look for) are not findings; report only genuine leftovers.
...
Example (POSIX shell): `git diff --cached | grep -nE '<pattern>'`. On Windows PowerShell use `git diff --cached | Select-String -Pattern '<pattern>'`. Scan added lines only (keep lines starting with `+`, drop the `+++` file header), map each hit back to its file and line through the hunk headers, and scan raw `git` output rather than output rewritten by a shell hook.
...
### Step 4b: Verify Nontrivial Changes

A change is nontrivial when the staged diff touches source or test code (not only docs, comments, or formatting) in more than one file, or changes more than about 50 lines. For a nontrivial change, before writing the message:

1. If a `/verify` skill is installed (it appears in this session's skill list, or `.claude/skills/verify/SKILL.md` exists at project or user level), recommend it: bundled workflow skills are user-invoked, so you cannot run it — tell the user the change is nontrivial, ask whether they want to run `/verify` first (default: yes), and if so stop here; they re-invoke `/commit` afterwards and you resume from Step 5 with the staged set intact. On that re-invocation, skip this step when the user says verification already ran (the hint alone does not count).
2. Otherwise, offer to run the project's own test command taken from its manifest (`package.json` scripts.test, `pyproject.toml`/`pytest.ini`, a `Makefile` test target, `*.csproj`, `Cargo.toml`): show the exact runner invocation you will execute (e.g. `npm test`, `pytest`, `dotnet test`, `cargo test`) and, when the manifest defines the script body, that body too, verbatim; run it only on explicit confirmation; never derive any part of it from `$ARGUMENTS` or the hint; if the staged diff modifies that manifest or the test configuration, say so before running.
3. If neither applies or the user declines, skip.

Verification runs against the working tree, which may include unstaged changes outside this commit — say so when that applies. After a run, re-check `git diff --cached --stat`; if the staged set changed, stop and show the difference. Record the outcome (passed / failed, user chose to proceed / failed, aborted / skipped: reason, including "user declined") for Step 7. A failure never blocks the commit by itself; the user decides.
...
3. Pass the message as data, never through shell interpolation. POSIX: `git commit -F - <<'EOF'` … `EOF` (quoted delimiter, so `$`, backticks and quotes in the hint are literal). PowerShell: normalize line endings to LF, write the message to a temp file outside the repo as UTF-8 without BOM with `[System.IO.File]::WriteAllText($path, $msg)`, run `git commit -F $path`, then delete the file. Never place the message in a double-quoted argument.
...
- Verification outcome from Step 4b
...
This skill is deliberately single-context. A commit is a short, sequential, state-mutating workflow: every decision (what to stage, what the message says) depends on the exact current index state, and the safety checks are deterministic pattern scans, not judgment calls that benefit from a second perspective. Delegating any step would add latency and a stale-state risk without improving the result. Recommending `/verify` or running a user-confirmed test command in Step 4b is a pre-commit check whose result the user acts on, not delegation of the commit decision; `/commit` spawns no agents and calls no router.
